...
export function retrieveError(retrieveError) {
  return { type: 'STORE_UPDATE_RETRIEVE_ERROR', retrieveError };
}

export function retrieveLoading(retrieveLoading) {
  return { type: 'STORE_UPDATE_RETRIEVE_LOADING', retrieveLoading };
}

export function retrieveSuccess(retrieved) {
  return { type: 'STORE_UPDATE_RETRIEVE_SUCCESS', retrieved };
}
...
export function updateError(updateError) {
  return { type: 'STORE_UPDATE_UPDATE_ERROR', updateError };
}

export function updateLoading(updateLoading) {
  return { type: 'STORE_UPDATE_UPDATE_LOADING', updateLoading };
}

export function updateSuccess(updated) {
  return { type: 'STORE_UPDATE_UPDATE_SUCCESS', updated };
}
...
export function mercureOpen(eventSource) {
  return { type: 'STORE_UPDATE_MERCURE_OPEN', eventSource };
}

export function mercureMessage(retrieved) {
  return dispatch => {
    if (1 === Object.keys(retrieved).length) {
      dispatch({ type: 'STORE_UPDATE_MERCURE_DELETED', retrieved });
      return;
    }

    dispatch({ type: 'STORE_UPDATE_MERCURE_MESSAGE', retrieved });
  };
}
